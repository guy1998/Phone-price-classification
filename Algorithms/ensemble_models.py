...
def apply_ensemble_fe_resolution(type, normalization_type, train_size):
    data_tuple = data_splitter("Datasets/fe_resolution_" + normalization_type + "_dataset.csv"
                                                     , train_size)
    if type == "forest":
        ensemble(17, data_tuple)
    else:
        ensemble_log(17, data_tuple)


def apply_ensemble_fe_screen_size(type, normalization_type, train_size):
    data_tuple = data_splitter("Datasets/fe_screen_size_" + normalization_type + "_dataset.csv"
                               , train_size)
    if type == "forest":
        ensemble(19, data_tuple)
    else:
        ensemble_log(19, data_tuple)


def apply_ensemble_normal(type, normalization_type, train_size):
    data_tuple = data_splitter("Datasets/" + normalization_type + "_dataset.csv"
                               , train_size)
    if type == "forest":
        ensemble(20, data_tuple)
    else:
        ensemble_log(20, data_tuple)


def apply_ensemble_raw_data(type, train_size):
...
def ensemble_model(dataset="raw", normalization_type="raw", train_size=0.8):
    if dataset == "fe_resolution":
        apply_ensemble_fe_resolution('forest', normalization_type, train_size)
    elif dataset == "fe_screen_size":
        apply_ensemble_fe_screen_size('forest', normalization_type, train_size)
    elif dataset == "normal":
        apply_ensemble_normal('forest', normalization_type, train_size)
    elif dataset == "raw":
        apply_ensemble_raw_data('forest', train_size)


def ensemble_model_logistic_regression(dataset="raw", normalization_type="raw", train_size=0.8):
    if dataset == "fe_resolution":
        apply_ensemble_fe_resolution('log', normalization_type, train_size)
    elif dataset == "fe_screen_size":
        apply_ensemble_fe_screen_size('log', normalization_type, train_size)
    elif dataset == "normal":
        apply_ensemble_normal('log', normalization_type, train_size)
    elif dataset == "raw":
        apply_ensemble_raw_data('log', train_size)